...
export default function ProductDetails({ productID, productSKU }) {
    const productdata = useRef(productInfo);

    const [item_count, setItem_count] = useState(0);
    const [selectedColor, setSelectedColor] = useState(productdata.colors[0]);
    const [selectedSize, setSelectedSize] = useState(productdata.sizes[0]);

    const stock_number = (selectedColor.stock_size > selectedSize.stock_size) ? selectedSize.stock_size : selectedColor.stock_size;
    ...
    return (<>
        <div onClick={hideQuickview} id="box-container" className="fixed top-0 left-0 w-full h-screen flex items-center justify-center z-50 bg-slate-200 bg-opacity-60 invisible" >
            <div onClick={(e) => e.stopPropagation()} id="box" className="relative opacity-0 w-0 flex-row sm:flex p-1.5 md:p-2.5 aspect-[1/2] sm:aspect-[8/5] bg-white transition-all duration-700 z-50 rounded-xl">
                    ...
                    <div className="max-sm:text-xl text-lg md:text-xl 2md:text-2xl xl:text-3xl sm:pr-3">
                        ...
                        <div className="mt-3 mb-8">
                            {(productdata.price != productdata.discount_price || productdata.discount_price == 0) ?
                                <>
                                    <span className="text-gray-400 max-sm:text-xl text-base md:text-lg 2md:text-xl xl:text-2xl pl-3">
                                        <del>
                                                    ...
                                                    <RadioGroup.Label as="span">
                                                        <Image
                                                            ...
                                                            className={classNames('absolute inset-0 rounded-md',
                                                                (color.stock_size > 0) ? 'opacity-100' : 'opacity-30'
                                                            )}
                                                        />
                                                    </RadioGroup.Label>
                                                    {color.inStock ? (
                    ...
                    <div className={`${roboto_slab.className} my-6 font-serif`}>
                        ...
                        <div className={`${pt_serif.className} text-base`}>
                            <ul className="list-disc pl-7 pt-4 font-lighter">
                                {productdata.point_description.map((description) => (
                                    <li>{description}</li>
                                ))}
                            </ul>
                            <p className="text-gray-800 mt-5"> <strong><big className="pr-2">Tags:</big></strong>
                                {productdata.tag_names.map((tag) => (
                                    <>{tag}, </>
                                ))}
                            </p>
                        </div>
